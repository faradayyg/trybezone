...
<script>
var feed_number_init = 30
	function load_more_feeds(div,how_many,btn,type)
	{
		start = feed_number_init
		feed_number_init +=how_many
		$.post('handles/load_more_follow.php',{type:type,username:'<?php echo $_GET['user'] ?>',user:user,token:token,start:start,stop:how_many},function(data){
			$('#f_append').append(data)
		}).success(function(){
