<?php
	...
	$username = sanitize($_SESSION['user']);
		...
		feeds.username WHERE subscriptions.username='$_SESSION[user]' ORDER BY feeds.stamp DESC LIMIT 0,20";
		try
		{
			...
			if($query->rowCount() == 0)
			{
				$sql = "SELECT * FROM feeds WHERE username = '$_SESSION[user]'  ORDER BY feeds.stamp DESC LIMIT 0,20";
				$query = $connection ->query($sql) ;
				$query->setFetchMode(PDO::FETCH_ASSOC);
				if($query->rowCount() == 0)
				{
					echo "<script>$('#gghhtt').hide()</script>
					<div style='padding:30px;line-height:35px;'>
					<h2 align='center' style='line-height:35px;'><center><i style='font-size:2em' class='ion-android-sad'></i></center><br />There are No posts to View,<br /> Try following
