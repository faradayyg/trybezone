...
INNER JOIN feeds ON subscriptions.subs_to = feeds.username
WHERE subscriptions.username='$username' ORDER BY feeds.stamp DESC LIMIT $start,$stop";
try
{
	...
	if($query->rowCount() == 0)
	{
		$sql = "SELECT * FROM feeds WHERE username = '$username'  ORDER BY feeds.stamp DESC LIMIT $start,$stop";
		$query = $connection ->query($sql) ;
		$query->setFetchMode(PDO::FETCH_ASSOC);
	}
	foreach($query as $r)
	{
		...
		if($r['type'] == 'status')
		{
			...
			FROM posts INNER JOIN users on users.username = posts.username INNER JOIN profilepics on users.username =
			profilepics.username WHERE posts.id = '$r[uploadid]'  AND posts.visible = '1'";
